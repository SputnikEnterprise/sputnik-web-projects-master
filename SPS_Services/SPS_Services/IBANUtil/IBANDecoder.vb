...
Namespace IBANUtil
  ...
  ''' </summary>
  ''' <remarks>Converted to VB.Net with http://converter.telerik.com. In the IsIBAN an extension is implemented that checks the length of the iban.</remarks>
...
#Region "Public Methods"
    ...
    Public Function IsIBAN(ByVal iban As String) As Boolean
      ...
      Else
        ...
        ' ---Erweiterung: Zum Code von http://www.tsql.de/csharp/csharp_iban_validieren_iban_testen_iban_code um IBAN Länge zu prüfen.--
        If Not m_IBAN_Patterns.ContainsKey(LaenderCode) Then
          Return False
        End If

        Dim ibanPattern = m_IBAN_Patterns(LaenderCode)

        Dim lengthIBAN As Integer = mysIBAN.Length()
        Dim expectedLength As Integer = (4 + ibanPattern.LengthBankID + ibanPattern.LengthAccountNumber)

        If lengthIBAN <> expectedLength Then
          Return False
        End If
        ' ---Ende Erweiterung---
