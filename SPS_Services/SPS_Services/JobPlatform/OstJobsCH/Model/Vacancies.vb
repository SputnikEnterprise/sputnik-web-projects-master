...
Namespace JobPlatform.OstJobsCH
  ...
  ''' <summary>
  ''' Represents a bunch of ostjobCh job vacancies.
  ''' </summary>
    ...
    Public Function ToXDoc() As XDocument
      ...
      If IsDataValidForXml Then
        ...
        Dim xDoc As New XDocument(
               ...
               New XElement("list", New XAttribute("version", "1"), (From vac In m_Vacancies Select vac.ToXElement())))
        Return xDoc
...
#Region "Pivate Functions"
    ...
    Private Function CheckVacancy(ByVal vacancy As Vacancy) As Boolean
      ...
      If Not vacancy.IsDataValidForXml Then
        m_ValidationErrors.Append(String.Format("CustomerGuid={0},VakNr={1}. Errors={2}", vacancy.CustomerGuid, vacancy.VakNr, vacancy.ValidationErrors))
        Return False
      End If
